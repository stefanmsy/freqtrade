#!/usr/bin/env python3
"""
Comprehensive analysis script for BTC Price Direction Strategy v1.12 with 2-class classification
...
def load_all_predictions():
    """Load predictions from all model files for v1.12"""
    model_path = Path("user_data/models/btc_price_direction_v1.12_2class_strict")
...
def analyze_predictions(predictions, price_data):
    """Analyze prediction distribution and quality for v1.12"""

    print("=" * 80)
    print("BTC PRICE DIRECTION STRATEGY v1.12 - 2-CLASS STRICT FILTERING ANALYSIS")
    print("=" * 80)
    ...
    # Threshold analysis for v1.12 (50% confidence, 60% probability)
    ...
    # Current v1.12 thresholds (50% confidence, 60% probability)
    current_passing = (
    ...
    # Compare with v1.11 thresholds
    v1_11_passing = (
        (predictions_with_signals["prob_diff"] > 0.30)
        & (predictions_with_signals["max_prob"] > 0.55)
    ).sum()

    print(f"\nCOMPARISON WITH v1.11 THRESHOLDS (30% confidence, 55% probability):")
    print(
        f"  v1.11 would pass: {v1_11_passing} predictions ({v1_11_passing / len(predictions_with_signals) * 100:.1f}%)"
    )
    print(
        f"  v1.12 passes: {current_passing} predictions ({current_passing / len(predictions_with_signals) * 100:.1f}%)"
    )
    print(f"  Change: {current_passing - v1_11_passing} predictions")
    ...
    if current_passing < 10:
        ...
        print("  → Consider reducing confidence threshold to 0.4")
        print("  → Consider reducing probability threshold to 0.55")
    elif current_passing > 200:
        print("  ⚠️  Too many predictions passing current thresholds")
        print("  → Consider increasing confidence threshold to 0.6")
        print("  → Consider increasing probability threshold to 0.65")
    else:
    ...
    high_conf = predictions_with_signals[predictions_with_signals["prob_diff"] > 0.5].head(10)
    if len(high_conf) > 0:
...
def create_visualizations(predictions):
    """Create visualizations for the v1.12 analysis"""
    ...
    fig.suptitle(
        "BTC Price Direction Strategy v1.12 - 2-Class Strict Filtering Analysis", fontsize=16
    )
    ...
    axes[0, 1].axvline(x=0.5, color="red", linestyle="--", label="v1.12 threshold (0.5)")
    axes[0, 1].axvline(x=0.3, color="orange", linestyle="--", label="v1.11 threshold (0.3)")
    axes[0, 1].set_xlabel("Probability Difference (Confidence)")
    axes[0, 1].set_ylabel("Frequency")
    axes[0, 1].set_title("Confidence Distribution (2-class)")
    axes[0, 1].legend()

    # 3. Probability distribution
    axes[1, 0].hist(predictions_with_signals["max_prob"], bins=30, alpha=0.7, edgecolor="black")
    axes[1, 0].axvline(x=0.6, color="red", linestyle="--", label="v1.12 threshold (0.6)")
    axes[1, 0].axvline(x=0.55, color="orange", linestyle="--", label="v1.11 threshold (0.55)")
    axes[1, 0].set_xlabel("Maximum Probability")
    axes[1, 0].set_ylabel("Frequency")
    axes[1, 0].set_title("Probability Distribution (2-class)")
    axes[1, 0].legend()
    ...
    # Save the plot
    output_path = Path("user_data/report/v1.12_2class_strict_analysis.png")
...
def main():
    ...
    print(f"\n✅ v1.12 2-Class Strict Filtering Analysis complete!")
